...
if (!isset($_GET['userId'])) {
    $totalSearchResults = BlogDB::totalSearchPostCount($_GET['search']);
    $totalSearchPages = ceil($totalSearchResults / $postsPerPage);
} elseif ($totalSearchResults = BlogDB::getSearchPostCount($_GET['userId'], $_GET['search'])) {
    $totalSearchPages = ceil($totalSearchResults / $postsPerPage);
}
...
?>
    <div class="container">
        <?php if (isset($_GET['userId']) && isset($_GET['search'])):
            $posts = BlogDB::searchByUser($_GET['userId'], $postsPerPage, $_GET['search'], $_GET['page']);
            ...
            <?php endforeach ?>
        <?php else:
            $posts = BlogDB::totalSearchPosts($_GET['search'], $postsPerPage, $_GET['page']);
            foreach ($posts as $key => $blog):
                foreach ($blog as $post):?>

                    <div class="post">
                        <h1 class="post-header"><?php echo $post['title'] ?>
                            <?php $currentUser = BlogDB::getUserById($key) ?>
                            <?php $postPage = ceil (BlogDB::getPostPosition($key, $post, $postsPerPage) / $postsPerPage) ?>
        ...
        <nav>
            <?php if ($pagination = BlogDB::renderPagination($totalSearchPages, $currentPage)) { ?>
                <ul class="pagination">
